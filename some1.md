...
static void mesh_example_info_store(void)
{
    ...
    err = nvs_set_blob(NVS_HANDLE, NVS_KEY, &store, sizeof(store));
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to store data in NVS");
    }

    err = nvs_commit(NVS_HANDLE);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to commit data in NVS");
    }
}
