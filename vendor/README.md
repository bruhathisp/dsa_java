                                             ...
                                             esp_ble_mesh_prov_cb_param_t *param)
{
    ...
    ESP_LOGI(TAG, "AppKey added successfully, now binding it to the model...");
    if (param->provisioner_add_app_key_comp.err_code == 0) {
        ...
        ESP_LOGE(TAG, "Failed to add AppKey, error code: %d", param->provisioner_add_app_key_comp.err_code);
    }
    break;
...
void example_ble_mesh_send_vendor_message(bool resend)
{
    ...
    // Log the destination address and context
    ESP_LOGI(TAG, "Sending message to 0x%04x with net_idx 0x%04x, app_idx 0x%04x",
             ctx.addr, ctx.net_idx, ctx.app_idx);

    const char *message = "Hello";
    uint8_t tid = store.vnd_tid++;

    err = esp_ble_mesh_client_model_send_msg(vendor_client.model, &ctx, opcode,
            strlen(message) + 1, (uint8_t *)message, MSG_TIMEOUT, true, MSG_ROLE);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to send vendor message 0x%06" PRIx32, opcode);
        ESP_LOGE(TAG, "Failed to send to 0x%04x with net_idx 0x%04x, app_idx 0x%04x",
                 ctx.addr, ctx.net_idx, ctx.app_idx);
        return;
    }

    ESP_LOGI(TAG, "Sent Hello message with tid: %d", tid);
    mesh_example_info_store(); /* Store proper mesh example info */
}
...
void app_main(void)
{
    ...
    xTaskCreate(send_hello_task, "send_hello_task", 4096, NULL, 5, NULL);
}
...
case ESP_BLE_MESH_MODEL_OPERATION_EVT:
    if (param->model_operation.opcode == ESP_BLE_MESH_VND_MODEL_OP_SEND) {
        char *received_message = (char *)param->model_operation.msg;
        ESP_LOGI(TAG, "Received message: %s", received_message);

        if (strcmp(received_message, "Hello") == 0) {
            ESP_LOGI(TAG, "Received Hello message, sending response...");

            const char *response = "Hello Ack";
            esp_err_t err = esp_ble_mesh_server_model_send_msg(&vnd_models[0],
                    param->model_operation.ctx, ESP_BLE_MESH_VND_MODEL_OP_STATUS,
                    strlen(response) + 1, (uint8_t *)response);
            if (err) {
                ESP_LOGE(TAG, "Failed to send Hello Ack message");
            }
        }
    }
    break;
    ...
    case ESP_BLE_MESH_MODEL_SEND_COMP_EVT:
        ...
        ESP_LOGI(TAG, "Sent message 0x%06" PRIx32, param->model_send_comp.opcode);
        break;
